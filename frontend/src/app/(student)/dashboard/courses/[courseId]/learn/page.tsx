...
// purpose : Load course-level uploaded documents for the student learning page.
import {
  getUploadedFileUrl,
  getCourseUploadedFiles,
  UploadedFileResponse,
} from '@/lib/services/uploaded-file.service'
...
// purpose : Convert supported YouTube URLs into embeddable player URLs.
...
// purpose : Convert backend resource paths into absolute browser-accessible URLs.
...
// purpose : Format watched video duration into a readable MM:SS value.
...
export default function CourseLearningPage() {
  ...
  // purpose : Store documents uploaded directly at the course level.
  ...
  // Load Course, Course Files, Chapters, Lessons & Saved Progress
  useEffect(() => {
    ...
    const loadRealCourseData = async () => {
      try {
        ...
        // purpose : Fetch documents uploaded directly to the entire course.
        try {
          const courseFilesRes = await getCourseUploadedFiles(numericId)

          setCourseFiles(courseFilesRes || [])
        } catch (e) {
          console.error(
            'Failed fetching course-level uploaded files:',
            e
          )
        ...
        if (rawChapters && rawChapters.length > 0) {
          ...
          for (const chap of rawChapters) {
            ...
            try {
              ...
              if (lesRes && lesRes.items) {
                for (const l of lesRes.items) {
                  ...
                  try {
                    ...
                    console.error(
                      'Failed fetching resources for lesson',
                      l.id,
                      e
                    )
                  }
              ...
              console.error(
                'Failed fetching lessons for chapter',
                chap.id,
                e
              )
            }
            ...
            const isChapCompleted =
              ...
              loadedLessons.every((l) => l.completed)
          ...
          if (
            ...
            loadedChapters[0].lessons.length > 0
          ) {
  ...
  const isCurrentLessonCompleted =
    ...
    (activeLessonId
      ? completedLessonsRef.current.has(activeLessonId)
      : false)
  ...
  const youtubeEmbedUrl = videoResource
    ? getYouTubeEmbedUrl(videoResource.url)
    : null
  ...
  const triggerLessonCompletion = async (
    ...
    try {
      ...
      console.error(
        'Failed saving lesson completion:',
        err
      )
    }
  }
  ...
  const handleVideoTimeUpdate = () => {
    ...
    if (current > maxWatchedPositionRef.current + 3) {
      ...
      setSeekWarning(
        'You skipped part of this lesson. Please watch the skipped content before continuing.'
      )
    ...
    const pct = Math.min(
      100,
      Math.round(
        (maxWatchedPositionRef.current / duration) * 100
      )
    )
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(() => {
      if (
        ytPlayerRef.current &&
        typeof ytPlayerRef.current.getCurrentTime ===
          'function'
      ) {
        try {
          const curr =
          ...
          const dur =
            ytPlayerRef.current.getDuration() || 1
          ...
          if (dur > 0) {
            ...
            if (
              curr >
              maxWatchedPositionRef.current + 3
            ) {
            ...
            const pct = Math.min(
              100,
              Math.round(
                (maxWatchedPositionRef.current / dur) *
                  100
              )
            )
  ...
  const completedCount = allLessons.filter(
    (l) =>
      l.completed ||
      completedLessonsRef.current.has(l.id)
  ).length
  ...
  const handleNextOrFinish = () => {
    if (
      ...
      setActiveLessonId(
        allLessons[activeLessonIndex + 1].id
      )
    } else {
      router.push(
        `/dashboard/courses/${courseId}/done?lessonId=${activeLessonId}`
      )
    }
  }
            ...
            chapters.map((chap, cIdx) => (
              ...
              >
                <div className="flex items-start gap-2">
                  <div
                    className={`size-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-[10px] font-bold ${
                      chap.completed
                ...
                <div className="ml-6 space-y-1">
                  {chap.lessons.map(
                    (les, lIdx) => {
                      ...
                      const isDone =
                        les.completed ||
                        completedLessonsRef.current.has(
                          les.id
                        )
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-xl transition-all text-xs flex items-center gap-2 ${
                            isActive
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                curriculum panel to begin learning.
              </p>
            </div>
          ) : (
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-lg space-y-2">
                  <div className="aspect-video w-full bg-black relative flex items-center justify-center">
                    {youtubeEmbedUrl ? (
                      ...
                      videoResource.file_path ? (
                      <video
                        key={`${activeLessonId}-${getResourceUrl(
                          videoResource.file_path ||
                            videoResource.url
                        )}`}
                        ref={videoRef}
              ...
              {/* purpose : Display documents uploaded for the entire course separately from lesson resources. */}
                  ...
                  <div className="space-y-3">
                    {courseFiles.map((file) => {
                      ...
                      return (
                        <div
                          key={file.id}
                          className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-xl border border-border p-4 hover:bg-muted/30 transition-colors"
                        >
                          <div className="flex min-w-0 items-center gap-3">
                            <div className="p-2.5 rounded-xl bg-red-500/10 text-red-600 flex-shrink-0">
                          ...
                          <a
                            ...
                            className="ml-0 sm:ml-4 inline-flex shrink-0 items-center justify-center gap-2 rounded-md border border-border px-3 py-2 text-sm font-medium hover:bg-muted transition-colors"
                          >
              ...
              <div className="space-y-4">
                ...
                0 ? (
                  <div className="rounded-2xl border border-dashed border-border p-6 text-center text-xs text-muted-foreground">
                    No additional study files attached
                    to this lesson yet.
                  </div>
                ) : (
              ...
              <div className="pt-6 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-xl px-6 py-3 text-xs font-bold shadow-md transition-all ${
                    isCurrentLessonCompleted
