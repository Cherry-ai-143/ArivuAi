...
export function TeacherTopNav() {
  ...
  const fullName = currentUser?.full_name || 'Teacher'
  const firstName = fullName.split(' ')[0]
  const designation = profileDetails?.designation || (currentUser?.role ? currentUser.role.charAt(0).toUpperCase() + currentUser.role.slice(1) : 'Teacher')
    ...
    <nav className="border-b border-border bg-card/80 backdrop-blur-md sticky top-0 z-40 shadow-sm">
      <div className="mx-auto flex h-16 max-w-full items-center justify-between gap-4 px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            type="button"
            onClick={() => router.push('/teacher-dashboard/settings?tab=notifications')}
          ...
          <DropdownMenu>
            ...
            >
              ...
              <DropdownMenuGroup>
                ...
                <DropdownMenuItem
                  onClick={() => router.push('/teacher-dashboard/profile?edit=true')}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium cursor-pointer hover:bg-primary/10 hover:text-primary transition-colors"
                >
